...
impl DataForge {
    ...
    pub fn record_to_xml(&self, path: &str) -> Result<String> {
        let record_idx = self.path_to_record()
            .get(path)
    ...
    pub fn record_to_xml_by_index(&self, index: usize) -> Result<String> {
        if index >= self.record_definitions().len() {
            return Err(Error::InvalidDataForge(format!("Record index {} out of range", index)));
        }
    ...
    pub fn all_records_to_xml(&self) -> Result<Vec<(String, String)>> {
        let mut results = Vec::new();
        
        for (path, &idx) in self.path_to_record() {
            match self.record_to_xml_by_index(idx) {
                Ok(xml) => results.push((path.clone(), xml)),
                Err(e) => {
...
impl<'a> XmlContext<'a> {
    ...
    fn build_struct_xml(
        ...
        let data_offset = match self.df.struct_to_data_offset().get(&struct_index) {
            Some(&offset) => self.df.data_offset() + offset + (struct_def.record_size as u64 * variant_index as u64),
    ...
    fn read_attribute_value(
        ...
        match prop.data_type {
            DataType::Boolean => {
                ...
                Ok(format!(" {}=\"{}\"", escaped_name, if val { "1" } else { "0" }))
            }
            DataType::Int8 => {
            ...
            DataType::String => {
                ...
                Ok(format!(" {}=\"{}\"", escaped_name, DataForge::escape_xml(&val)))
            }
            DataType::Locale => {
                let offset = cursor.read_u32::<LittleEndian>()?;
                let val = self.df.read_text_at_offset(offset as u64)?;
            ...
            DataType::StrongPointer | DataType::WeakPointer => {
                ...
                if struct_idx == 0xFFFFFFFF {
                    ...
                    Ok(format!(" {}=\"{}[{:04X}]\"", escaped_name, struct_name, variant_idx))
                }
            }
            DataType::Class => {
            ...
            DataType::Unknown(t) => {
                Ok(format!(" {}=\"Unknown type {}\"", escaped_name, t))
            }
        }
    }
    ...
    fn read_array_item(
        ...
        match prop.data_type {
            DataType::Boolean => {
                let val: bool = self.df.read_value_at(self.df.boolean_value_offset() + index)?;
                Ok(format!("<Boolean>{}</Boolean>", if val { "1" } else { "0" }))
            }
            DataType::Int8 => {
            ...
            DataType::Int16 => {
                let val: i16 = self.df.read_value_at(self.df.int16_value_offset() + index * 2)?;
                Ok(format!("<Int16>{}</Int16>", val))
            }
            DataType::Int32 => {
                let val: i32 = self.df.read_value_at(self.df.int32_value_offset() + index * 4)?;
                Ok(format!("<Int32>{}</Int32>", val))
            }
            DataType::Int64 => {
                let val: i64 = self.df.read_value_at(self.df.int64_value_offset() + index * 8)?;
                Ok(format!("<Int64>{}</Int64>", val))
            }
            DataType::UInt8 => {
                let val: u8 = self.df.read_value_at(self.df.uint8_value_offset() + index)?;
                Ok(format!("<UInt8>{}</UInt8>", val))
            }
            DataType::UInt16 => {
                let val: u16 = self.df.read_value_at(self.df.uint16_value_offset() + index * 2)?;
                Ok(format!("<UInt16>{}</UInt16>", val))
            }
            DataType::UInt32 => {
                let val: u32 = self.df.read_value_at(self.df.uint32_value_offset() + index * 4)?;
                Ok(format!("<UInt32>{}</UInt32>", val))
            }
            DataType::UInt64 => {
                let val: u64 = self.df.read_value_at(self.df.uint64_value_offset() + index * 8)?;
                Ok(format!("<UInt64>{}</UInt64>", val))
            }
            DataType::Single => {
                let val: f32 = self.df.read_value_at(self.df.single_value_offset() + index * 4)?;
                Ok(format!("<Single>{}</Single>", val))
            }
            DataType::Double => {
                let val: f64 = self.df.read_value_at(self.df.double_value_offset() + index * 8)?;
                Ok(format!("<Double>{}</Double>", val))
            }
            DataType::String => {
                let str_offset: u32 = self.df.read_value_at(self.df.string_value_offset() + index * 4)?;
                let val = self.df.read_text_at_offset(str_offset as u64)?;
                Ok(format!("<String>{}</String>", DataForge::escape_xml(&val)))
            }
            DataType::Locale => {
                let str_offset: u32 = self.df.read_value_at(self.df.locale_value_offset() + index * 4)?;
                let val = self.df.read_text_at_offset(str_offset as u64)?;
                Ok(format!("<Locale>{}</Locale>", DataForge::escape_xml(&val)))
            }
            DataType::Enum => {
                let str_offset: u32 = self.df.read_value_at(self.df.enum_value_offset() + index * 4)?;
                let val = self.df.read_text_at_offset(str_offset as u64)?;
            ...
            DataType::Class => {
                ...
                let content = self.build_struct_xml(prop.index as u32, first_index + offset as u32, &struct_name)?;
                if content.is_empty() {
                    Ok(format!("<{} />", DataForge::escape_xml(&struct_name)))
                } else {
                    Ok(format!("<{}{}></{}>", DataForge::escape_xml(&struct_name), content, DataForge::escape_xml(&struct_name)))
                }
            }
            DataType::StrongPointer => {
                ...
                if struct_idx == 0xFFFFFFFF {
                    ...
                    let struct_name = self.df.get_struct_name(struct_idx as usize)?;
                    let content = self.build_struct_xml(struct_idx, variant_idx as u32, &struct_name)?;
                    if content.is_empty() {
                        Ok(format!("<{} />", DataForge::escape_xml(&struct_name)))
                    } else {
                        Ok(format!("<{}{}></{}>", DataForge::escape_xml(&struct_name), content, DataForge::escape_xml(&struct_name)))
            ...
            DataType::WeakPointer => {
                ...
                if struct_idx == 0xFFFFFFFF {
                    ...
                    let struct_name = self.df.get_struct_name(struct_idx as usize)?;
                    Ok(format!("<WeakPointer>{}[{:04X}]</WeakPointer>", struct_name, variant_idx))
                }
            }
            DataType::Reference => {
            ...
            DataType::Unknown(t) => {
                Ok(format!("<Unknown>type {}</Unknown>", t))
            }
        }
    }
}
