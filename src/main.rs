...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Some(Commands::Dcb { dcb_file, output, merge, info }) => {
...
fn convert_dcb(
    ...
    let data = fs::read(dcb_path)
        .with_context(|| format!("Failed to read {}", dcb_path.display()))?;
    
    let df = DataForge::parse(&data)
        .with_context(|| "Failed to parse DataForge file")?;
    
    // Show info
    println!("\nDataForge Info:");
    println!("  Version: {}", df.header.file_version);
    println!("  Legacy format: {}", df.header.is_legacy);
    println!("  Struct definitions: {}", df.header.struct_definition_count);
    println!("  Property definitions: {}", df.header.property_definition_count);
    ...
    if merge {
        // Export all records to single XML file (optional behavior)
        ...
        // Default: Export each record to separate file (like upstream)
        ...
        println!("\nExporting {} records to separate files in {}...", df.record_count(), output_dir.display());
        let pb = ProgressBar::new(df.record_count() as u64);
        pb.set_style(ProgressStyle::default_bar()
            .template("{spinner:.green} [{elapsed_precise}] [{bar:40.cyan/blue}] {pos}/{len} ({eta})")
            .unwrap()
        ...
        pb.finish_with_message("Done");
        println!("\nExported {} records ({} failed) to {}", success, failed, output_dir.display());
    }
